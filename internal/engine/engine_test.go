...
func setupTestTmpDir(t *testing.T) (string, engine.Engine, persistence.DataStoreFactory) {
	...
	ds := client.NewDataStoreFactory(&config.Config{
		DataDir: path.Join(tmpDir, ".blackdagger", "data"),
		DAGs:    path.Join(tmpDir, ".blackdagger", "dags"),
	})
...
func TestGetStatusRunningAndDone(t *testing.T) {
		...
		&sock.Config{
			...
			HandlerFunc: func(w http.ResponseWriter, r *http.Request) {
				status := model.NewStatus(
					...
					scheduler.Status_Running, 0, nil, nil)
	...
	require.NoError(t, err)
	require.Equal(t, scheduler.Status_Running, st.Status)

	_ = socketServer.Shutdown()

	st, err = e.GetCurrentStatus(ds.DAG)
	require.NoError(t, err)
	require.Equal(t, scheduler.Status_None, st.Status)
}
...
func TestUpdateStatus(t *testing.T) {
	...
	st := testNewStatus(d.DAG, requestId,
		scheduler.Status_Success, scheduler.NodeStatus_Success)
...
func TestUpdateStatusError(t *testing.T) {
	...
	status := testNewStatus(d.DAG, requestId,
		scheduler.Status_Error, scheduler.NodeStatus_Error)
...
func TestStop(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return st.Status == scheduler.Status_Running
	...
	require.Eventually(t, func() bool {
		...
		return st.Status == scheduler.Status_Cancel
	}, time.Millisecond*1500, time.Millisecond*100)
}
...
func TestCreateNewDAG(t *testing.T) {
	...
	d, err := cl.Load(path.Join(tmpDir, ".blackdagger", "dags", id+".yaml"), "")
	require.NoError(t, err)
...
func TestRenameDAG(t *testing.T) {
	...
	// TODO: fixme to use mock
	loc := path.Join(tmpDir, ".blackdagger", "dags", id+".yaml")

	_, err = e.GetStatus(loc)
	require.NoError(t, err)

	// TODO: fixme
	loc2 := path.Join(tmpDir, ".blackdagger", "dags", id+"_renamed.yaml")
