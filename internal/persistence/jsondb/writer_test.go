...
func TestWriteStatusToFile(t *testing.T) {
	...
	status := model.NewStatus(d, nil, scheduler.Status_Running, 10000, nil, nil)
...
func TestWriteStatusToExistingFile(t *testing.T) {
	...
	status := model.NewStatus(d, nil, scheduler.Status_Cancel, 10000, nil, nil)
	...
	require.NoError(t, err)
	require.Equal(t, data.Status.Status, scheduler.Status_Cancel)
	...
	data, err = db.FindByRequestId(d.Location, status.RequestId)
	require.NoError(t, err)
	require.Equal(t, data.Status.Status, scheduler.Status_Success)
	require.Equal(t, file, data.File)
}
